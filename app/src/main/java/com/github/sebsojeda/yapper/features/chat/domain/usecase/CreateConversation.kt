...
class CreateConversation @Inject constructor(
    ...
    operator fun invoke(participants: List<String>, content: String): Flow<Resource<Conversation>> = flow {
        emit(Resource.Loading())
        try {
            val conversation = chatManager.createConversation(participants, content).toConversation()
